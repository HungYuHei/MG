...
<div class="highlight"><pre>gem install rails
  ...
  describe "topic_favorite_tag" do
    ...
    it "should result when logined user did not favorite topic" do
      ...
      helper.topic_favorite_tag(topic).should == %(<a href="#" class="bookmark" data-id="#{topic.id}" onclick="return Topics.favorite(this);" rel="twipsy" title="收藏">收藏</a>)
    end

    it "should result when logined user favorited topic" do
      user.stub(:favorite_topic_ids).and_return([topic.id])
      helper.stub(:current_user).and_return(user)
      helper.topic_favorite_tag(topic).should == %(<a href="#" class="favorited" data-id="#{topic.id}" onclick="return Topics.favorite(this);" rel="twipsy" title="取消收藏">取消收藏</a>)
    end
